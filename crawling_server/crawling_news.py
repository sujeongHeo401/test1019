...
class run_crawling:
    ...
    def make_data_frame(self):
        page = self.eachPage
        url = "https://news.naver.com/main/main.naver?mode=LSD&mid=shm&sid1=100#&date=%2000:00:00&page=" + str(page)
        print("baase url : ", url)
        html = requests.get(url, headers=self.headers).text
        soup = BS(html, 'html.parser') if html !=None else None
        ranking_total = soup.find_all(class_='type06') if soup != None else None
        print("ranking_total: ", ranking_total)
...
def funcjj(single_page):
    print("안오 ㅏ??")
    a = run_crawling(single_page)
    a.make_data_frame()
