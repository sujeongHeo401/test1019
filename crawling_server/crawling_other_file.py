...
#엑셀로 저장하기 위한 변수
RESULT_PATH ='crawling_server'  #결과 저장할 경로
...
def crawler(maxpage,query,sort,s_date,e_date):
    print(maxpage,query,sort,s_date,e_date)
    print(type(maxpage),type(query),type(sort),type(s_date),type(e_date))
    ...
    while page <= maxpage_t:
        ...
        #모든 리스트 딕셔너리형태로 저장
        result= {"date" : date_text , "title":title_text , "title_c":title_text ,"media" : source_text ,"content": contents_text ,"src":link_text }  
        df = pd.DataFrame(result)  #df로 변환
        print(page)
        df['content_c'] = df.apply(clean_content_text, axis=1)
        df['word'] = ''
        ...
        df = df[df['media'] != 'EPA연합뉴스']    # 코리아헤럴드는 영어 제목임
        df = df[df['media'] != '주간경향']    # 주간경향은 같은 title이 많음
        df['title_c_neo4j'] = df.apply(clean_text, axis=1)
